...
# Reuse brokers already bound on this machine. Do not start a second Rabbit/Kafka.
...
# --- MediaMTX ---
$mtxYml = Join-Path $Root "backend\data\mediamtx-8554.yml"
$mtxBin = $env:MEDIAMTX_BIN
if (-not $mtxBin) {
  $guess = @(
    "D:\applicationPath\ffmpegPath\mediamtx.exe",
    (Join-Path $Root "tools\mediamtx.exe"),
    "mediamtx.exe"
  )
  foreach ($g in $guess) {
    if (Get-Command $g -ErrorAction SilentlyContinue) { $mtxBin = (Get-Command $g).Source; break }
    if (Test-Path $g) { $mtxBin = $g; break }
  }
}

if (-not (Test-PortOpen 8554)) {
  if (-not $mtxBin -or -not (Test-Path $mtxBin)) {
    Write-Warning "MediaMTX not found / port 8554 closed. Set MEDIAMTX_BIN or start mediamtx manually."
  } else {
    Write-Host "[mtx] starting $mtxBin"
    Start-Process -FilePath $mtxBin -ArgumentList $mtxYml -WorkingDirectory (Split-Path $mtxBin) -WindowStyle Minimized
    Start-Sleep -Seconds 1
  }
} else {
  Write-Host "[mtx] already listening on 8554"
}
